<template>
  <div class="project-details">
    ...
    <div class="project-header" :style="{ 
      background: `linear-gradient(135deg, ${project?.color || '#667eea'} 0%, ${adjustColor(project?.color || '#667eea', -20)} 100%)` 
    }">
      <div class="container">
        <div class="header-content">
          <div class="d-flex align-items-center">
            ...
            <div class="project-info ms-3">
              <h1 class="project-title">{{ project?.name || 'Projet' }}</h1>
              <div class="project-stats">
                ...
                <span class="stat-item">
                  ...
                  {{ doingTasks.length }} en cours
                </span>
              </div>
            </div>
          </div>
          <button class="btn-add-task" @click="showTaskModal = true">
    ...
    <div class="container mt-5">
      ...
      <div v-else class="kanban-grid">
        ...
        >
          <div class="column-header">
            <div class="column-title">
              <span class="column-icon todo-icon">
                <i class="bi bi-circle"></i>
              </span>
              <h3>À faire</h3>
              <span class="column-count">{{ todoTasks.length }}</span>
            </div>
            <p class="column-subtitle">Tâches à démarrer</p>
          </div>
          <div class="column-body">
            ...
            >
              ...
              <div class="task-footer">
                <span class="task-priority" :class="`priority-${task.priority || 'medium'}`">
            ...
            <div class="empty-state" v-if="todoTasks.length === 0">
              <i class="bi bi-inbox"></i>
              <p>Aucune tâche à faire</p>
              <button class="btn-add-column" @click="showTaskModal = true">
        ...
        >
          <div class="column-header">
            <div class="column-title">
              <span class="column-icon doing-icon">
                <i class="bi bi-clock"></i>
              </span>
              <h3>En cours</h3>
              <span class="column-count">{{ doingTasks.length }}</span>
            </div>
            <p class="column-subtitle">En progression</p>
          </div>
          <div class="column-body">
            <div 
            ...
            <div class="empty-state" v-if="doingTasks.length === 0">
              <i class="bi bi-inbox"></i>
        ...
        >
          <div class="column-header">
            <div class="column-title">
              <span class="column-icon done-icon">
                <i class="bi bi-check-circle"></i>
              </span>
              <h3>Terminé</h3>
              <span class="column-count">{{ doneTasks.length }}</span>
            </div>
            <p class="column-subtitle">Tâches complétées</p>
          </div>
          <div class="column-body">
            <div 
...
const createNewTask = async () => {
  ...
  try {
    ...
    const result = await projectStore.createTask(projectId.value, taskData)
    if (result.success) {
      closeModal()
      resetTaskForm()
    }
...
const updateTask = async () => {
  taskLoading.value = true
  try {
    await projectStore.updateTask(projectId.value, currentTask.id, currentTask)
    closeModal()
  } catch (error) {
    console.error('Error updating task:', error)
  } finally {
...
onMounted(async () => {
  loading.value = true
  try {
    unsubscribe.value = await projectStore.fetchTasks(projectId.value)
  } catch (error) {
...
.kanban-column[dragover] .column-body {
...
.btn-primary {
  background: linear-gradient(135deg, var(--primary-color, #667eea), var(--primary-dark, #5a67d8));
  color: white;
  position: relative;
  overflow: hidden;
}
